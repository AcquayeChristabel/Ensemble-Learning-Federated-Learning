...
class StackedEnsemble(nn.Module):
    def __init__(self, num_classes=200):
        ...
        self.models = task_models
        for model in self.models:
            model.fc = nn.Identity()  
        self.classifier = nn.Linear(512 * len(task_models), num_classes)  
...
def ensemble_prediction(test_example, 
                        ...
                        knn_model=None):
    predictions = [model.predict(test_example) for model in task_models]
    ...
    if aggregation_method == "random":
        ...
        correct_model_index = 0  
        prediction = predictions[correct_model_index]

    elif aggregation_method == "confidence":
        ...
        confidences = [max(np.softmax(pred['logits'])) for pred in predictions]
        prediction = predictions[np.argmax(confidences)]
        # raise NotImplementedError("confidence based selection not yet implemented")

    elif aggregation_method == "neighbor_majority":
        neighbors = knn_model.find_neighbors(test_example)
        votes = [pred for neighbor in neighbors for pred in predictions]
...
def main():
    ...
    def train_stack(model):
        ...
        if args.output_dir is not None:
            ...
            if accelerator.is_main_process:
                image_processor.save_pretrained(args.output_dir)
    stacked_ensemble = StackedEnsemble(num_classes=200)
    train_stack(stacked_ensemble)
